...
### Мои поездки (водитель / владелец)

- **URL**: `GET /api/trip/owned`
- **Auth**: **требуется аутентификация**. Важно: этот маршрут зарегистрирован **до** общего правила «все `GET /api/trip/**` публичны» (`WebSecurityConfig`), иначе список «своих» поездок оказался бы доступен всем подряд.
- **Query**:
...
- анонимный доступ к **`GET /api/trip/owned`** → redirect на логин (как для защищённых ресурсов)
- **`GET /api/trip/owned`**: фильтры `timeFilter`, пагинация
